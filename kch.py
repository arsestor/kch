...
st.caption('Если мы сможем определять, в каких из сообщений говорится о событии-катастрофе, исключая нерелевантные, например, сообщения о фильмах, мы сможем ускорить уведомление экстренных служб о чрезвычайных ситуациях и ускорить необходимую помощь')
...
st.caption('Ядром приложения является обученная модель, которая определяет являются ли введенное сообщение действительно сообщением об угрозе или нет.')
st.caption('Ниже, в поле ввода вы можете ввести сообщение и приложение сообщит является ли оно сообщением о ЧС.')
...
if (st.button('Определить')):
    ...
    if submission(clf_NB_TFIDF, test_tfidf) == 1:
        st.header(':red[Сообщение релевантно (сообщение о происшествии)]')
    else:
        st.header(':green[Сообщение не релевантно (сообщение не о происшествии)]')
